<?php
	$conn = new PDO('mysql:host=localhost;dbname=imamfati_forum','imamfati_root','2*Imam*Bagus'); //Membuat koneksi db
...
<div class="content">
	<?php
		...
		$dbc = new PDO('mysql:host=localhost;dbname=imamfati_forum','imamfati_root','2*Imam*Bagus'); //Membuat koneksi db
		...
		foreach ($query as $row) { //Perulangan untuk menampilkan topic
			...
			if ($owner == $_SESSION['id']){ //Jika pada perulangan saat ini, pemilik id topic sama dengan session id, maka bisa diedit
				...
				echo "<li class='title li'>Subject: {$row['subject']}</li>";
